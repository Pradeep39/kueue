...
## Historical: the Kueue-owned Apache integration (Architecture B)

How `pkg/controller/jobs/apachesparkapplication` handled Dynamic Allocation scaling before it was
removed. Kept because it is the reference for any reimplementation - the counterpart flows to the
proposed operator-owned design below, so the two can be read side by side to see where the work
moves.
...
to honour `spec.suspend` for the initial admission. That is the contrast with Architecture A below,
where four of nine lanes move into the operator.

Generated by `gen_apache_kueue_seq.py`. Its references are **split by lifetime**: symbols in the
generic Kueue packages are still live code and are resolved from source like the as-built pair, but
symbols in the removed `apachesparkapplication` package are **pinned to `f1d6092ab`**, the last
commit where it existed. Resolving those from source would fail the build on any branch where the
removal has landed, and a line number for deleted code is a historical fact rather than a live one.
...
— §4 is the annotation contract it depends on, §5 the Kueue machinery it reuses, §6 the Java
task list. Generated by `gen_apache_da_seq.py`, which imports the helpers from `gen_seq.py`.
...
is not drawn — it is two in-place patches, and the decrease-only admission exception that permits
the second already exists and keys off the *Workload's* `kueue.x-k8s.io/elastic-job` annotation,
so the operator can do it with no further Kueue change.
