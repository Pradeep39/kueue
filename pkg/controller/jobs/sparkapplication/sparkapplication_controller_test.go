...
func TestPodSets(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"dynamic allocation enabled, instances unset, falls back to initialExecutors": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
		...
		"dynamic allocation enabled, instances and initialExecutors unset, falls back to minExecutors": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
				DynamicAllocation(&sparkappv1beta2.DynamicAllocation{
					Enabled:      true,
			...
			want: []kueue.PodSet{
				...
				*utiltestingapi.MakePodSet("executor", 2).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
					InitContainers: []corev1.Container{},
					Containers: []corev1.Container{
		...
		"dynamic allocation enabled, instances explicitly set, takes precedence over initialExecutors": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstances(1).
				DynamicAllocation(&sparkappv1beta2.DynamicAllocation{
					Enabled:          true,
			...
			want: []kueue.PodSet{
				...
				*utiltestingapi.MakePodSet("executor", 1).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
					InitContainers: []corev1.Container{},
					Containers: []corev1.Container{
		...
		"dynamic allocation configured via sparkConf only, falls back to minExecutors": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
				SparkConf("spark.dynamicAllocation.enabled", "true").
				SparkConf("spark.dynamicAllocation.minExecutors", "6").
				SparkConf("spark.dynamicAllocation.maxExecutors", "12").Obj(),
			want: []kueue.PodSet{
				*utiltestingapi.MakePodSet("driver", 1).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
				...
				*utiltestingapi.MakePodSet("executor", 6).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
					InitContainers: []corev1.Container{},
					Containers: []corev1.Container{
		...
		"dynamic allocation configured via sparkConf only, initialExecutors takes precedence over minExecutors": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
				SparkConf("spark.dynamicAllocation.enabled", "true").
				SparkConf("spark.dynamicAllocation.initialExecutors", "4").
				SparkConf("spark.dynamicAllocation.minExecutors", "2").Obj(),
			want: []kueue.PodSet{
				*utiltestingapi.MakePodSet("driver", 1).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
				...
				*utiltestingapi.MakePodSet("executor", 4).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
					InitContainers: []corev1.Container{},
					Containers: []corev1.Container{
		...
		"dynamic allocation configured via sparkConf, structured minExecutors takes precedence over sparkConf": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
				SparkConf("spark.dynamicAllocation.minExecutors", "6").
				DynamicAllocation(&sparkappv1beta2.DynamicAllocation{
		...
		"dynamic allocation not enabled, instances unset, sparkConf minExecutors ignored": {
			sparkApp: testSparkApp.Clone().
				ExecutorInstancesUnset().
				SparkConf("spark.dynamicAllocation.minExecutors", "6").Obj(),
			want: []kueue.PodSet{
				*utiltestingapi.MakePodSet("driver", 1).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
				...
				*utiltestingapi.MakePodSet("executor", 0).PodSpec(corev1.PodSpec{
					NodeSelector:   map[string]string{},
					Tolerations:    []corev1.Toleration{},
					InitContainers: []corev1.Container{},
					Containers: []corev1.Container{
